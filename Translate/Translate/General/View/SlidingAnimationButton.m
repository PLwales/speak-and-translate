...
#import "SlidingAnimationButton.h"

static NSString *const cellIdentifier = @"SlidingAnimationButtonCell";

@interface SlidingAnimationButton () <UICollectionViewDelegateFlowLayout, UICollectionViewDataSource>

@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) UICollectionViewFlowLayout *layout;
@property (nonatomic, assign) CGFloat kButtonWdithSpace;
@property (nonatomic, assign) CGFloat kCollectionLeftOrRight;
@end
#define KWidthButtonWidthAddSpace   10
@implementation SlidingAnimationButton
...
-(UICollectionView*)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[SlidingAnimationButtonCell class] forCellWithReuseIdentifier:cellIdentifier];
...
-(void)setTitles:(NSArray *)titles {
    _titles = titles;
    CGFloat kWidthCount = 0;
    for (int i = 0; i<titles.count; i++) {
        NSString * title = titles[i];
        if (titles.count<=5) {
            CGSize size = [title sizeWithAttributes:@{NSFontAttributeName:self.selectedFont}];
            size = CGSizeMake(size.width+KWidthButtonWidthAddSpace, CGRectGetHeight(self.bounds));
            kWidthCount+=size.width;
        } else {
            self.kButtonWdithSpace = 0;
        }
    }
    if (kWidthCount<WIDTH_SCREEN) {
        self.kButtonWdithSpace = (WIDTH_SCREEN-kWidthCount-_cursorEdgeInsets.left-_cursorEdgeInsets.right)/(titles.count-1);
        if (titles.count==2) {
            self.kCollectionLeftOrRight = (WIDTH_SCREEN-kWidthCount-_cursorEdgeInsets.left-_cursorEdgeInsets.right)/6;
            self.kCollectionLeftOrRight = self.kCollectionLeftOrRight > 0?self.kCollectionLeftOrRight:0;
            self.kButtonWdithSpace = (WIDTH_SCREEN-kWidthCount-_cursorEdgeInsets.left-_cursorEdgeInsets.right-self.kCollectionLeftOrRight*2)/(titles.count-1);
        } else {
            self.kCollectionLeftOrRight = 0;
        }
        self.kButtonWdithSpace = self.kButtonWdithSpace > 0?self.kButtonWdithSpace:0;
    }
}
...
-(void)selectItemAtIndexPath:(NSIndexPath*)indexPath {
    SlidingAnimationButtonCell *cell = (SlidingAnimationButtonCell*)[self.collectionView cellForItemAtIndexPath:indexPath];
    cell.selected = YES;
...
{
    SlidingAnimationButtonCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellIdentifier forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didDeselectItemAtIndexPath:(NSIndexPath *)indexPath {
    SlidingAnimationButtonCell *cell = (SlidingAnimationButtonCell*)[collectionView cellForItemAtIndexPath:indexPath];
    cell.selected = NO;
}
...
{
    return UIEdgeInsetsMake(0, self.kCollectionLeftOrRight, 0, self.kCollectionLeftOrRight);
}
- (CGFloat)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout minimumInteritemSpacingForSectionAtIndex:(NSInteger)section {
    return self.kButtonWdithSpace;
}
...
/****************************SlidingAnimationButtonCell****************************************/



@interface SlidingAnimationButtonCell ()

@property (nonatomic, strong) UILabel *titleLabel;

@end

@implementation SlidingAnimationButtonCell
...
-(void)layoutSubviews
{
    ...
    _titleLabel.frame = CGRectIntegral(self.bounds);
}
